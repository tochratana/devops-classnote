> 1. When should you use each? (NodeSelector, Node Affinity)

Node Affinity is better because it is more flexible, supports soft and hard rules, and is suitable for production environments. NodeSelector is simpler but limited and mainly used for basic scenarios.

> 2. If don't want all service is decided which node to run and I want to run or cusomize all service which node to run, so how to do ?

We use NoteSeletor 